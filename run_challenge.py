...
def compile_code(source_folder):
    ...
    result = subprocess.run(
        ["mvn", "clean", "package"], capture_output=True, text=True, cwd=source_folder
    )
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".txt"):
            ...
            if result.returncode == 124:
                error_msg = (
                    f"Execution timed out after {MAX_RUNNING_TIME} for {input_file}"
                )
                print(error_msg)
                ...
                print(result.stderr)
                raise RuntimeError(
                    f"Execution failed for {input_file}: {result.stderr}"
                )
